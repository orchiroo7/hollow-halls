...
func _process(_delta: float) -> void:
	...
	hud.set_camera(rig.view_key(), rig.is_side(), rig.is_free())
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("quit_game"):
		...
		rig.toggle_free()
	elif event.is_action_pressed("lang_toggle"):
		switch_language()
...
func switch_language() -> void:
	Lang.toggle()
	hud.relabel()
	for node in get_tree().get_nodes_in_group("signpost"):
		node.text = Lang.t(node.get_meta("key"))
	if touch != null:
		touch.relabel()
